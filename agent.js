/**
 * CoffeeAddict - Advanced AI Assistant
...
class CoffeeAddict {
    ...
    async init() {
        try {
            ...
            console.log(`CoffeeAddict v${this.version} initialized successfully`);
        } catch (error) {
            console.error('Failed to initialize CoffeeAddict:', error);
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'CoffeeAddict', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'How can I assist you?');
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.agentFlow = new CoffeeAddict();
});
